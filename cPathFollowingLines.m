function [Drone,Path,rho] = cPathFollowingLines(Drone,Path,Vmax)
...
Xr([7 8 9 12]) = [V*cos(alpha)*cos(beta); V*cos(alpha)*sin(beta); V*sin(alpha); Path.dpsi];
% A.pPos.Xr = Xr;
% Modelo Cinemático
