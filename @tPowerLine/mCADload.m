...
uav.pCAD.obj{1} = loadawobj('PowerLine_ElectricPylon_v2.obj');
uav.pCAD.mtl{1} = loadawmtl('PowerLine_ElectricPylon_v2.mtl');
...
%------------------------------------------------
% Ajustando Dimensões:
% vExt = minmax(uav.pCAD.obj{1}.v);
% mExt = mean(vExt,2);
% 
% for ii = 1:3
%     uav.pCAD.obj{1}.v(ii,:) = (uav.pCAD.obj{1}.v(ii,:) - mExt(ii))/1e1; % Era /1000, mas fica muito pequeno...
% end

% Ajustando a orientação e posição iniciais:
uav.pCAD.obj{1}.v = [1 0 0; 0 0 -1; 0 1 0]*uav.pCAD.obj{1}.v;   % Rx,theta = [1 0 0;
                                                                %             0 cos(theta) -sen(theta);
                                                                %             0 sen(theta) cos(theta)];


uav.pCAD.obj{1}.v = [0 1 0; -1 0 0; 0 0 1]*uav.pCAD.obj{1}.v;   % Rz,psi   = [cos(psi) -sen(psi) 0;
                                                                %             sen(psi)  cos(psi) 0 ;
                                                                %             0 0 1];


                                                                
uav.pCAD.obj{1}.v(3,:) = uav.pCAD.obj{1}.v(3,:) - min(uav.pCAD.obj{1}.v(3,:))*ones(1,length(uav.pCAD.obj{1}.v(3,:)));

uav.pCAD.obj{1}.v(1,:) = uav.pCAD.obj{1}.v(1,:) + (15.00)*ones(1,length(uav.pCAD.obj{1}.v(1,:)));
uav.pCAD.obj{1}.v(2,:) = uav.pCAD.obj{1}.v(2,:) + (29.00)*ones(1,length(uav.pCAD.obj{1}.v(2,:)));
